-- ============================================================
-- 1) OPCIONES DE ESTADO DE LÍNEA: "SIN PROGRAMACIÓN" + NOTA EN DETENIDA
-- 2) EL SUFIJO DE FAMILIA DEL SABOR NO SE MUESTRA PARA CLÁSICOS/ESPECIALES
-- ============================================================
--
-- (1) El estado continuo de una línea (lineas_estado, ver
--     20260943090000) tenía DETENIDA / LISTA / CIP / CAMBIO_PRESENTACION.
--     Se agrega SIN_PROGRAMACION (no se va a conectar al futuro módulo de
--     Programación — es solo un estado que carga el supervisor a mano), y
--     una nota libre "observacion" (máx. 140) que solo aplica a DETENIDA
--     — falla u observación que se muestra en el Panel de Producción al
--     lado de la línea.
--
-- (2) sabor_nombre venía siempre como "{sabor} ({familia})"
--     (20260930090000). El sufijo entre paréntesis solo hace falta
--     cuando el mismo nombre de sabor se repite en varias familias
--     (Jucosa, Selecto, Premium). Para "Clasicos" y "Especiales" se saca
--     — queda "Manzana" a secas. Se centraliza en sabor_display() y se
--     usa en todos los sabor_nombre de turno_json(). El frontend ya
--     aplica el mismo criterio (saborSinFamiliaOculta / nombreSaborConFamilia)
--     para que valga aun antes de aplicar esta migración.
-- ============================================================

-- ------------------------------------------------------------
-- sabor_display(): nombre de sabor para mostrar.
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- lineas_estado: nueva condición + columna observacion.
-- ------------------------------------------------------------
alter table lineas_estado drop constraint if exists lineas_estado_condicion_check;
alter table lineas_estado add constraint lineas_estado_condicion_check
  check (condicion in ('DETENIDA', 'LISTA', 'CIP', 'CAMBIO_PRESENTACION', 'SIN_PROGRAMACION'));

alter table lineas_estado add column if not exists observacion text
  check (observacion is null or char_length(observacion) <= 140);

-- ------------------------------------------------------------
-- cambiar_condicion_linea(): gana p_observacion (solo se guarda con
-- DETENIDA; cualquier otra condición limpia la nota).
-- ------------------------------------------------------------
drop function if exists cambiar_condicion_linea(text, uuid, text, text);
...
-- ------------------------------------------------------------
-- turno_json(): idéntica a 20260968090000, con dos cambios:
--   - todos los sabor_nombre pasan por sabor_display()
--   - lineas_estado expone observacion
-- ------------------------------------------------------------
