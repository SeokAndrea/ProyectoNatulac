-- ============================================================
-- SNAPSHOT "TANQUES ENCONTRADOS" — para el acta en PDF
-- ============================================================
-- El acta necesita mostrar "cómo estaban los tanques al empezar"
-- (encontrado) vs "cómo quedaron" (dejado = estado actual de
-- recepcion_tanques). Ese dato "encontrado" NO se toma al crear el
-- turno (ahí todavía es solo lo heredado del turno anterior, sin
-- revisar) — se toma recién cuando el supervisor TERMINA de
-- confirmar/corregir los 3 tanques en el paso de revisión de Status
-- (ver ConfirmarEstadoTanque.tsx, que ya se autodesactiva por tanque
-- una vez confirmado_inicio_en queda seteado). Una vez capturado, el
-- snapshot queda fijo para siempre — no se vuelve a pisar.
-- ============================================================

alter table turnos add column tanques_encontrados jsonb;

-- ------------------------------------------------------------
-- capturar_tanques_encontrados_si_completo(): candado + snapshot.
-- No hace nada si ya se capturó antes, o si todavía falta confirmar
-- algún tanque. Se llama al final de confirmar_estado_tanque() y
-- cambiar_condicion_tanque() — los únicos dos caminos por los que un
-- tanque llega a tener confirmado_inicio_en.
-- ------------------------------------------------------------
create or replace function capturar_tanques_encontrados_si_completo(p_turno_id uuid)
returns void
language plpgsql
security definer
...
as $$
begin
  if exists (select 1 from turnos where id = p_turno_id and tanques_encontrados is not null) then
    return;
  end if;

  if exists (select 1 from recepcion_tanques where turno_id = p_turno_id and confirmado_inicio_en is null) then
    return;
  end if;

  update turnos
  set tanques_encontrados = (
...
grant execute on function capturar_tanques_encontrados_si_completo(uuid) to anon, authenticated;

-- ------------------------------------------------------------
-- confirmar_estado_tanque(): agrega la llamada al final.
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- cambiar_condicion_tanque(): agrega la llamada al final (idéntica a
-- la última definición en 20260945090000_porcentaje_tanque_por_lote.sql,
-- solo con el "perform" agregado antes del return).
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- turno_json(): expone tanques_encontrados (passthrough directo del
-- jsonb ya armado, sin reconstruirlo).
-- ------------------------------------------------------------
