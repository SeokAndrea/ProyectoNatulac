-- ============================================================
-- "EDITADO POR": quién corrigió Producto Terminado después de registrado
-- ============================================================
-- registrar_producto_terminado() pisaba usuario_id en cada UPDATE
-- (on conflict), así que si un Super Admin corregía la fila de un
-- supervisor, usuario_id pasaba a ser el del admin — se perdía quién
-- lo cargó originalmente. Se separa en dos datos:
--   - usuario_id: SIEMPRE el que lo registró primero (Fernando), ya no
--     se vuelve a pisar en ediciones posteriores.
--   - editado_por / editado_en: quién lo corrigió después y cuándo —
--     solo lo toca corregir_producto_terminado_auditoria() (la
--     corrección desde Editar Turno), nunca el registro normal del
--     supervisor en su propio turno.
-- ============================================================

alter table producto_terminado add column editado_por uuid references usuarios (id);
alter table producto_terminado add column editado_en timestamptz;

-- ------------------------------------------------------------
-- registrar_producto_terminado(): ya no pisa usuario_id en el UPDATE
-- (solo lo setea el INSERT inicial).
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- corregir_producto_terminado_auditoria(): además de corregir, marca
-- editado_por/editado_en con el admin que hizo el cambio.
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- turno_json(): expone quién lo registró originalmente y quién lo
-- corrigió después (si corresponde).
-- ------------------------------------------------------------
