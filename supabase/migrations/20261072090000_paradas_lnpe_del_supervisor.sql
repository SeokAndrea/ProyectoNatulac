-- ============================================================
-- PARADAS: el supervisor también registra Línea no programada (LNPE)
-- ============================================================
-- Lista del supervisor (dueño, 2026-09-21): Programadas (PPL#), Línea no programada
-- (LNPEL#-#) y Operacionales (OPL#-#), más el Tiempo ocioso. Mantenimiento registra el
-- resto (equipos, Suministro, Equipo de Proceso, Domino). Corrige 20261071, que había
-- dejado LNPE del lado de Mantenimiento.
-- Los tipos «Línea No Programada/…» conservan solo el segundo texto.
-- ============================================================

update paradas_tipos set nombre = 'Disponibilidad de Ventas' where codigo = 'LINEA_NO_PROG_VENTAS';
update paradas_tipos set nombre = 'Falta de Insumos/Paletas' where codigo = 'LINEA_NO_PROG_INSUMOS_PALETAS';
update paradas_tipos set nombre = 'Falta de Espacio Almacenamiento' where codigo = 'LINEA_NO_PROG_ESPACIO_ALMACEN';
-- OPL#-2 se queda como «Insumos (Prueba Industrial)» (20261068), por decisión del dueño.
...
begin
  ...
  -- Programadas, Línea no programada (LNPE) y Operacionales las registra el supervisor.
  if v_area is distinct from 'PRUEBAS' and v_tipo.familia in ('PROGRAMADA', 'EXTERNA', 'OPERACIONAL') then
