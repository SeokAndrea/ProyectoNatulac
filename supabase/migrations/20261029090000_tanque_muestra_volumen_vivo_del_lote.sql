-- ============================================================
-- turno_json(): la tarjeta del tanque muestra el volumen VIVO del lote
-- ============================================================
-- Desde costura 2 (20261018) Producción ya no escribe en
-- `recepcion_tanques` — `registrar_producto_terminado` solo baja
-- `preparaciones.volumen_l`. Consecuencia: `recepcion_tanques.volumen_l`
-- queda CONGELADO en el valor que le puso `liberar_lote`, y la tarjeta
-- del tanque (Preparación / Líneas / Status / Panel) muestra ese número
-- clavado durante toda la corrida. El supervisor ve el tanque "lleno" y
-- vuelve a correr la línea sobre él (caso Turno 3 de Javier, Lote 0009
-- corrido 7 veces).
--
-- Cambio: en el bloque `tanques[]`, `volumen_l` lee el volumen VIVO del
-- lote abierto (`prep_t.volumen_l`, ya viene por el join que da
-- `volumen_inicial_l`) cuando el tanque está LISTO/STANDBY con un lote
-- sin cerrar. En cualquier otro caso (tanque sucio/limpio/en
-- preparación, o lote ya cerrado) sigue devolviendo `rt.volumen_l`.
--
-- NO reabre costura 2: Producción sigue sin escribir `recepcion_tanques`.
-- Solo cambia QUÉ campo lee la vista. `estado_planta_actual`,
-- `turno_de_fecha_tipo` y `turno_abierto_ahora` reusan `turno_json`, así
-- que el Panel de Producción también queda arreglado. Frontend: nada —
-- todos los consumidores leen `volumen_l`.
--
-- Cuerpo idéntico a `20261011` salvo el `case` de `volumen_l` en el
-- bloque `tanques[]`. `create or replace`, sin cambio de firma.
-- ============================================================
  ...
  select jsonb_build_object(
    ...
    'tanques', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        -- volumen VIVO del lote abierto (el join ya lo trae); si no hay
        -- lote abierto -> el valor de recepcion_tanques como antes.
        'volumen_l', case
          when rt.condicion in ('LISTO', 'STANDBY')
               and prep_t.id is not null
               and prep_t.cerrado_en is null
          then coalesce(prep_t.volumen_l, rt.volumen_l)
          else rt.volumen_l
        end,
